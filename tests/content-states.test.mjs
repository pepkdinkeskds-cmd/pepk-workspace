...
test("all active page modules retain the shared status language", async () => {
  for (const page of pageModules) {
    ...
    assert.match(content, /status\.js\?v=0\.9\.5-quality-06/, `${page}.js must load the shared status module`);
...
test("changed pages load the Quality 06 module cache key", async () => {
  for (const page of htmlPages) {
    const content = await source(`${page}.html`);
    assert.match(content, /js\/pages\/[^"]+\.js\?v=0\.9\.5-quality-06/, `${page}.html must use the Quality 06 page-module cache key`);
  }
});
