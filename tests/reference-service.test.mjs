...
test("every portal entry point uses the same versioned bridge", async () => {
  for (const page of ["home", "contribute", "information", "monev"]) {
    ...
    assert.match(script, /submission-portal-bridge\.js\?v=0\.9\.5-quality-05/);
  }
});

test("all public pages load the Quality 05 cache keys", async () => {
  ...
  for (const page of pages) {
    ...
    assert.match(html, /js\/pages\/[^"]+\.js\?v=0\.9\.5-quality-05/);
  }
});
